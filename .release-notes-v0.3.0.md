# v0.3.0 — M3 技能市场 + 体验与稳定性修复批次

> 「辰夕 WorkAgent」阶段发布：M1~M3 全部落地（账号/模型/文件/SSE 对话闭环 → HITL 参数补全 + Docker 沙箱 + 产物面板 → 技能市场 + MinIO 技能仓库 + @ 唤起），并含 2026-09-16 体验与稳定性修复批次。
...
- 后端重新打包前**先停掉运行中的 JVM**，否则运行中 `mvn package` 会导致旧进程 NoClassDefFoundError
- agentscope-java 2.0.3-SNAPSHOT 需先安装到本地仓库（见 README 构建命令）
